...
import { View, Text, TextInput, ScrollView, TouchableOpacity, Alert, ActivityIndicator, StyleSheet, Pressable } from "react-native";
...
const SubmittedCounselingList = () => {
    ...
    const fetchCounselingForms = async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${baseURL}/getAllUserSubmittedCounseling`, {
            ...
            {loading ? (
                ...
                <ScrollView>
                    {filteredForms.map((item, index) => {
                        ...
                        return (
                            <Pressable key={item._id} onPress={() => handleCardPress(item._id)}>
                                <Box
                                    bg="white"
                                    p={4}
                                    mb={3}
                                ...
                                >
                                    ...
                                    <VStack mt={2}>
                                        <Text>
                                            <Text fontWeight="bold">Purpose:</Text> {item.purpose || "N/A"}
                                        </Text>
                                        <Text>
                                            <Text fontWeight="bold">Contact Number:</Text> {item.contactNumber || "N/A"}
                                        </Text>
                                        <Text>
                                            <Text fontWeight="bold">Counseling Date:</Text> {item.counselingDate ? new Date(item.counselingDate).toLocaleDateString() : "N/A"}
                                        </Text>
                                        <Text>
                                        ...
                                        <Text>
                                            <Text fontWeight="bold">Contact Person Number:</Text> {item.contactPerson?.contactNumber || "N/A"}
                                        </Text>
                                        <Text>
                                            <Text fontWeight="bold">Relationship:</Text> {item.contactPerson?.relationship || "N/A"}
                                        </Text>
                                    </VStack>
                                </Box>
                            </Pressable>
