...
import BaptismMassChecklist from "./BaptismMassChecklist";
...
const MySubmittedMassBaptismForm = () => {
  const route = useRoute();
  const navigation = useNavigation();
  const { massBaptismId } = route.params;
  console.log(massBaptismId)
  ...
  const fetchBaptismDetails = async () => {
    try {
      const response = await axios.get(`${baseURL}/massBaptism/getUserMassBaptismForm/${massBaptismId}`);
  ...
  const submitCancel = async () => {
    ...
    try {
      await axios.post(
        `${baseURL}/massBaptism/declineMassBaptism/${massBaptismId}`,
        { reason: cancelReason },
    ...
    <View style={{ flex: 1, backgroundColor: "#f4f4f4" }}>
      ...
      {activeTab === 3 && (
        <View style={{ flex: 1, padding: 16 }}>
          <BaptismMassChecklist massBaptismId={massBaptismId} />
        </View>
      )}
    </View>
  );
...
export default MySubmittedMassBaptismForm;
