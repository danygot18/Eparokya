...
import UserMassWeddingChecklist from "./UserMassWeddingChecklist";
...
const SubmittedMassWeddingDetails = () => {
  const route = useRoute();
  const { massWeddingId } = route.params;
  ...
  const fetchWedding = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${baseURL}/massWedding/getUserMassWeddingForm/${massWeddingId}`);
      setWedding(res.data);
  ...
  const handleCancel = async () => {
    ...
    try {
      await axios.post(`${baseURL}/massWedding/declineMassWedding/${massWeddingId}`, {
        reason: cancelReason,
        user: "User",
      });
    ...
    <View style={{ flex: 1, backgroundColor: "#f4f4f4" }}>
      ...
      {activeTab === 3 && (
        <View style={{ flex: 1, padding: 16 }}>
          <UserMassWeddingChecklist massWeddingId={massWeddingId} />
        </View>
      )}
    </View>
  );
...
export default SubmittedMassWeddingDetails;
