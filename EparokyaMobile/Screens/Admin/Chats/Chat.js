...
const Chat = ({ route }) => {
    const { userId } = route.params; // Ensure userId is passed correctly
    ...
    const getChat = async () => {
        try {
            const { data } = await axios.get(`${baseURL}/chat/getMessage/${userId._id}/${user._id}`, {
                headers: {
    ...
    console.log('API Response:', userId._id);
