...
const BaptismMassForm = ({ navigation }) => {
  ...
  const [selectedBaptismDate, setSelectedBaptismDate] = useState(null);

  // Fetch active baptism dates from API (getActiveDate functionality)
  useEffect(() => {
    const fetchBaptismDates = async () => {
      try {
        const category = "Baptism";
        const response = await axios.get(
          `${baseURL}/getActiveDatesByCategory/${category}`,
          {
            headers: {
        ...
        if (response.data.length > 0) {
          ...
          setBaptismDate(new Date(response.data[0].date));
          setBaptismTime(new Date(`1970-01-01T${response.data[0].time}`));
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    // Use selected baptism date if available
    if (selectedBaptismDate) {
      formData.append("baptismDate", selectedBaptismDate.date);
      formData.append("baptismTime", selectedBaptismDate.time);
      formData.append("baptismDateTime", selectedBaptismDate._id);
    } else {
    ...
    try {
      const response = await axios.post(`${baseURL}/massBaptism/massBaptismCreate`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (response.status === 201) {
        Alert.alert("Success", "Mass Baptism form submitted successfully.");
        setPhone("");
        setChild({ fullName: "", dateOfBirth: new Date(), placeOfBirth: "", gender: "" });
        setParents({
          fatherFullName: "",
          placeOfFathersBirth: "",
          motherFullName: "",
        ...
        setNinong({ name: "", address: "", religion: "" });
        setNinang({ name: "", address: "", religion: "" });
        setNinongGodparents([]);
        setNinangGodparents([]);
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 20 }}>
      ...
      {/* Baptism DateTime from active dates */}
      <View style={styles.section}>
        <Text style={styles.label}>Baptism Date and Time</Text>
        <TextInput
          value={
            selectedBaptismDate
              ? `${formatDate(new Date(selectedBaptismDate.date))} - ${formatTime(
                new Date(`1970-01-01T${selectedBaptismDate.time}`)
              )}`
              : ''
          }
          style={styles.input}
      ...
      <View style={styles.section}>
        <Text style={styles.subHeader}>Ninong (Principal Godfather) Details *</Text>
        <TextInput
          style={styles.input}
          placeholder="Name"
          value={ninong.name}
      ...
      <View style={styles.section}>
        <Text style={styles.subHeader}>Ninang (Principal Godmother) Details *</Text>
        <TextInput
          style={styles.input}
          placeholder="Name"
          value={ninang.name}
...
export default BaptismMassForm;