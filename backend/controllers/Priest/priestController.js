...
exports.createPriest = async (req, res) => {
  try {
    ...
    const guestPriestBool = GuestPriest === 'true' || GuestPriest === true;
    const activeBool = isActive === 'true' || isActive === true;
    const retiredBool = isRetired === 'true' || isRetired === true;
    const availableBool = isAvailable === 'true' || isAvailable === true;
    const transferedBool = isTransfered === 'true' || isTransfered === true;
    ...
    if (
      ...
      !Seminary ||
      !ordinationDate ||
      (!guestPriestBool && !parishDurationYear)
    ) {
    ...
    const newPriest = new Priest({
      ...
      Seminary,
      ordinationDate,
      parishDurationYear: guestPriestBool ? undefined : parishDurationYear,
      isActive: activeBool,
      isAvailable: availableBool,
      isRetired: retiredBool,
      isTransfered: transferedBool,
      GuestPriest: guestPriestBool,
      image: {
    ...
    const formatToTimeString = (datetimeStr) => {
    ...
    if (guestPriestBool) {
      newPriest.guestDetails = {
        attended: attended || null,
        date: guestDate ? new Date(guestDate) : null,
        time: formatToTimeString(guestTime),
      };

      console.log("Guest Details to Save:", newPriest.guestDetails);
    }

    const savedPriest = await newPriest.save();
    console.log("Priest Saved:", savedPriest);
    res.status(201).json({ success: true, priest: savedPriest });
  } catch (error) {
    console.error("Error creating priest:", error.message);
    res.status(500).json({
      success: false,
      message: 'Error creating priest',
      error: error.message,
    });
  }
};
