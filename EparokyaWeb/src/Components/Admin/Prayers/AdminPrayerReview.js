...
useEffect(() => {
  ...
  if (searchTerm) {
    filtered = filtered.filter((form) =>
      form.userId?.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }
  ...
  return (
    <div style={{ display: "flex", height: "100vh" }}>
      ...
      <div style={{ flex: 1, padding: "20px", overflowY: "auto" }}>
        ...
        <div className="adminPrayerRev-filters">
          <input
            type="text"
            placeholder="Search by name..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="adminPrayerRev-filter-input"
          />
          <button
            className={`adminPrayerRev-filter-button ${
        ...
        <div className="adminPrayerRev-others">(others)</div>
