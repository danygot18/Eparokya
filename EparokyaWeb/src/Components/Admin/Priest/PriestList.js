...
const PriestList = () => {
  ...
  const handleEdit = (priest) => {
    ...
    console.log("Edit priest:", priest);
  };

const handleDelete = async (priestId) => {
  if (window.confirm("Are you sure you want to delete this priest?")) {
    try {
      const response = await fetch(`${process.env.REACT_APP_API}/api/v1/deletePriest/${priestId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (response.ok) {
        ...
        alert(data.message || 'Failed to delete priest');
      }
    } catch (error) {
    ...
    try {
      ...
      setPriests((prevPriests) =>
        prevPriests.map((priest) =>
          priest._id === priestId
            ? { ...priest, [field]: response.data[field] }
            : priest
        )
      );
          ...
          <div key={priest._id} className="priestList-item">
            ...
            <div className="priestList-image-container">
              <img
                ...
                alt={priest.fullName}
                className="priestList-image"
              />
            </div>
            <div className="priestList-details">
              <h2>{priest.fullName}</h2>
              <p className="priestList-year-range">
              ...
              <div className="priestList-field">
                Title:
                <input
                  className="priestList-field-value"
                  value={priest.title}
                  readOnly
                />
              </div>
              ...
              <div className="priestList-field">
                ...
                <input
                  className="priestList-field-value"
                  value={new Date(priest.birthDate).toLocaleDateString()}
                  readOnly
                />
              </div>
              <div className="priestList-field">
              ...
              <div className="priestList-field">
                ...
                <input
                  className="priestList-field-value"
                  value={new Date(priest.ordinationDate).toLocaleDateString()}
                  readOnly
                />
              </div>
