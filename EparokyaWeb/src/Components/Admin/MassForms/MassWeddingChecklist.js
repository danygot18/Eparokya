...
const MassWeddingChecklist = ({ massWeddingId }) => {
  ...
  const confirmSave = async () => {
    try {
      await axios.put(
        `${process.env.REACT_APP_API}/api/v1/massWedding/updateWeddingChecklist/${massWeddingId}`,
        checklist,
        { withCredentials: true }
      );
  ...
  const verifiedItems = Object.keys(checklist).filter(key => checklist[key] === true && key !== "notes");
  const unverifiedItems = Object.keys(checklist).filter(key => checklist[key] === false && key !== "notes");
            ...
            <Paper elevation={0} sx={{ p: 2, mb: 2, bgcolor: 'grey.100' }}>
              ...
              <List dense sx={{ maxHeight: 200, overflow: 'auto' }}>
                ...
                {verifiedItems.length === 0 && (
                  <ListItem>
                    <ListItemText primary="No items verified yet" />
                  </ListItem>
                )}
              </List>
            </Paper>
