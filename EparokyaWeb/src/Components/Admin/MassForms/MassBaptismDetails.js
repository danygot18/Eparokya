...
const MassBaptismDetails = () => {
    ...
    const fetchBaptismDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/massBaptism/getMassBaptism/${massBaptismId}`,
  ...
  useEffect(() => {
    const fetchAvailablePriests = async () => {
      try {
        ...
        console.log("Available priests response:", response.data);
        setAvailablePriests(response.data.priests);
      } catch (error) {
        console.error("Error fetching priests:", error);
      }
    };

    fetchAvailablePriests();
  }, []);
  ...
  const handleAdminNotes = async (e) => {
    ...
    if (!massBaptismId || massBaptismId.length !== 24) {
      toast.error("Invalid Mass Baptism ID.", {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API}/api/v1/massBaptism/adminMassAdditionalNotes/${massBaptismId}`,
        newAdminNote
      );
      ...
      console.error(
        "Error submitting additional notes:",
  ...
  const handleConfirm = async () => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API}/api/v1/massBaptism/${massBaptismId}/confirmMassBaptism`,
  ...
  const handleCancel = async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API}/api/v1/massBaptism/declineBaptism/${massBaptismId}`,
        { reason: cancelReason },
        { withCredentials: true }
      );

      toast.success("Baptism cancelled successfully!", {
        position: toast.POSITION.TOP_RIGHT,
      });
      setShowCancelModal(false);
    } catch (error) {
      toast.error(
        error.response?.data?.message || "Failed to cancel the baptism.",
        {
          position: toast.POSITION.TOP_RIGHT,
        }
      );
    ...
    <div className="baptism-details-page">
      ...
      <div className="baptism-details-box">
            ...
            <Card sx={{ flex: 1, marginBottom: 2 }}>
              <CardContent>
                ...
                <Typography>
                  ...
                  {baptismDetails?.userId?.phone || "N/A"}
                </Typography>
              </CardContent>
            </Card>
            <Card sx={{ flex: 1, marginBottom: 2 }}>
          ...
          <Box>
            ...
            <Card sx={{ flex: 1, marginBottom: 2 }}>
              <CardContent>
                ...
                <Typography>
                  <strong>Additional Ninongs:</strong>{" "}
                  {baptismDetails?.NinongGodparents?.map((gp) => gp.name).join(
                    ", "
                  ) || "N/A"}
                </Typography>
                <Typography>
                  <strong>Additional Ninangs:</strong>{" "}
        ...
        <div className="admin-section">
          ...
          <FormControl fullWidth>
            <InputLabel id="priest-label">Select Priest</InputLabel>
            <Select
              labelId="priest-label"
              id="priest"
              value={priest}
          ...
          <div className="house-comments-section">
            ...
            <div className="admin-comment">
              <p>
                ...
                {baptismDetails.cancellingReason.user === "Admin"
                  ? "Admin"
      ...
      <div className="wedding-checklist-container">
        ...
        <button
          onClick={() =>
            navigate(
              `/adminChat/${baptismDetails?.userId?._id}/${baptismDetails?.userId?.email}`
            )
          }
        >
          Go to Admin Chat
