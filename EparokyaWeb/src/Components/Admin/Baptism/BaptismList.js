...
const BaptismList = () => {
  ...
  const fetchBaptismForms = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API}/api/v1/baptismList`,
  ...
  // Filter and sort
  const filteredForms = baptismForms.filter((baptism) => {
    return (
      (filteredStatus === "All" || baptism.binyagStatus === filteredStatus) &&
      (searchTerm === "" ||
        baptism.child?.fullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        baptism.parents?.fatherFullName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        baptism.parents?.motherFullName?.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  });
