...
const CounselingList = () => {
  ...
  const fetchCounselingForms = async () => {
    try {
      ...
      console.log("Fetched Counseling Forms:", sortedForms);
      setCounselingForms(sortedForms);
      setFilteredForms(sortedForms);
    } catch (err) {
      setError(
        err.response?.data?.message || "Error fetching counseling forms."
      );
    } finally {
      setLoading(false);
    }
  ...
  const filterForms = () => {
    ...
    if (activeFilter !== "All") {
      filtered = filtered.filter(
        (form) => form.counselingStatus === activeFilter
      );
    }
            ...
            {filteredForms.map((item, index) => (
              <div
                ...
                className={`counseling-card ${
                  item.counselingStatus?.toLowerCase() || ""
                }`}
