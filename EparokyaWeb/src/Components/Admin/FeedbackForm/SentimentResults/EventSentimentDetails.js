    ...
    const fetchSentimentDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/getEventSentimentById/${id}`
        );
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Container sx={{ marginLeft: "240px", paddingTop: "20px" }}>
        ...
        <Box sx={{ marginBottom: "20px" }}>
          ...
          <Typography variant="body1">
            <strong>Event Type:</strong> {sentiment.eventTypeId?.name || "N/A"}{" "}
            {/* Access the name property */}
          </Typography>
          <Typography variant="body1">
...
export default EventSentimentDetails;
