...
const navigate = useNavigate();
    ...
    const fetchSentiments = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/getAllActivitySentiment`
        );
        setSentiments(response.data);
      } catch (err) {
        console.error("Error fetching sentiments:", err);
        setError("Failed to fetch sentiments.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCardClick = (id) => {
    navigate(`/admin/ActivitytSentimentDetail/${id}`);
  };
          ...
          <Table>
            ...
            <TableBody>
              {sentiments.map((sentiment) => (
                <TableRow
                  key={sentiment._id}
                  onClick={() => handleCardClick(sentiment._id)}
                  sx={{ cursor: "pointer", "&:hover": { backgroundColor: "#f5f5f5" } }}
                >
                  <TableCell>{sentiment.userId?.name || "N/A"}</TableCell>
                  <TableCell>{sentiment.activityTypeId?.name || "N/A"}</TableCell>
...
export default ActivitySentimentList;