...
const ActivitySentimentDetails = () => {
  const { id } = useParams();
    ...
    const fetchSentimentDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/getActivitySentimentById/${id}`
        );
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <Container sx={{ marginLeft: "240px", paddingTop: "20px" }}>
        ...
        <Box sx={{ marginBottom: "20px" }}>
          ...
          <Typography variant="body1">
            <strong>Activity Type:</strong> {sentiment.activityTypeId?.name || "N/A"}
          </Typography>
          <Typography variant="body1">
...
export default ActivitySentimentDetails;