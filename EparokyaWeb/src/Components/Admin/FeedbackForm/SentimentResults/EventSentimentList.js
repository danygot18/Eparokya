...
const EventSentimentList = () => {
    ...
    const fetchSentiments = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API}/api/v1/getAllEventSentiment`
        );
  ...
  const handleCardClick = (id) => {
    navigate(`/admin/EventSentimentDetails/${id}`); 
  };
            ...
            <TableBody>
              {sentiments.map((sentiment) => (
                ...
                >
                  ...
                  <TableCell>{sentiment.eventTypeId?.name || "N/A"}</TableCell>
...
export default EventSentimentList;