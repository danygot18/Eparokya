...
import { FaHeart, FaRegHeart, FaThumbsUp, FaRegThumbsUp, FaReply, FaPaperPlane } from 'react-icons/fa';
...
const AnnouncementDetails = () => {
    ...
    const fetchAnnouncement = async () => {
        try {
            ...
            const res = await axios.get(`${process.env.REACT_APP_API}/api/v1/getAnnouncement/${id}`);
    ...
    const addComment = async () => {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_API}/api/v1/${id}/announcementComment`, { text: commentText }, config);
            if (res.data.message === 'Your comment contains a profane word, please revise it.') {
    ...
    const addReply = async (commentId, replyText) => {
        ...
        try {
            ...
            if (res.data.message === 'Your reply contains a profane word, please revise it.') {
                toast.error(res.data.message);  
            } else {
                setComments(prevComments =>
                    prevComments.map(comment =>
            ...
            <div className="announcement-container">
                {announcement ? (
                    <>
                        ...
                        <p dangerouslySetInnerHTML={{ __html: announcement.richDescription }}></p>
                          ...
                          {announcement.images.length > 1 ? (
                            ...
                            {announcement.images.map((img, index) => (
                                <SwiperSlide key={img.public_id}>
                                    <img src={img.url} alt="Announcement" className="announcement-image" />
                                </SwiperSlide>
                            ))}
                        </Swiper>
                        ) : (
                            announcement.images.length === 1 && (
                                <img src={announcement.images[0].url} alt="Announcement" className="announcement-image" />
                            )
                        )}
                        {announcement.videos.length > 0 && announcement.videos.map(video => (
                            ...
                            {comments.map(comment => (
                                <div key={comment._id} className="comment-box">
                                    <div className="comment-header">
                                        <img src={comment.user.avatar?.url} alt={comment.user.name} className="comment-avatar" />
                                        <div>
                                            <p className="comment-user">{comment.user.name}</p>
                                            <p className="comment-date">{format(new Date(comment.dateCreated), 'PPP')}</p>
                                        </div>
                                    </div>
                                    <p className="comment-text">{comment.text}</p>

                                    <div className="comment-footer">
                                        <button onClick={() => toggleLikeComment(comment._id)} className="icon-btn">
                                            {comment.likedBy.includes(user._id) ? <FaThumbsUp /> : <FaRegThumbsUp />}
                                        </button>
                                        <span>{comment.likedBy.length} Likes</span>

                                        <button onClick={() => toggleReplies(comment._id)} className="icon-btn">
                                            <FaReply />
                                        </button>
                                        ...
                                        <div className="replies-section">
                                            {comment.replies.map(reply => (
                                                <div key={reply._id} className="reply-box">
                                                    <div className="comment-header">
                                                        <img src={reply.user.avatar?.url} alt={reply.user.name} className="comment-avatar" />
                                                        <div>
                                                            <p className="comment-user">{reply.user.name}</p>
                                                            <p className="comment-date">{format(new Date(reply.dateCreated), 'PPP')}</p>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="reply-input-container">
                                                ...
                                                />
                                                <button onClick={() => addReply(comment._id, replyText)} className="submit-reply-btn">
                                                    <FaPaperPlane size={18} />
                                                </button>
                                                <button onClick={() => setReplyText('')} className="clear-reply-btn">
                                                    Clear
                                                </button>
                                            </div>
                                        </div>
