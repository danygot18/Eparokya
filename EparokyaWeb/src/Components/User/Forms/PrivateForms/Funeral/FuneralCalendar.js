...
const FuneralCalendar = () => {
  ...
  const fetchFunerals = useCallback(async () => {
    try {
      ...
      console.log('Funerals loaded:', data);
    } catch (err) {
  ...
  return (
    <Box sx={{
      ...
      <Box sx={{
        ...
        <Typography sx={{mb: 2}}>
          View scheduled funeral services. For assistance, please contact the parish office.
        </Typography>
          ...
          <Paper elevation={3} sx={{
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <Typography sx={{ fontSize: isMobile ? '0.9rem' : '1rem' }}>
                <strong>Deceased:</strong> {selectedEvent.deceased || 'N/A'}
              </Typography>
